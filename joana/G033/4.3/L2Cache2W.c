...
void initCache()
{
    ...
    for (int i = 0; i < L2_SIZE / (BLOCK_SIZE * WAYS); i += 1)
    {
        for (int j = 0; j < WAYS; j++)
        {
            cache.l2.ways[i].lines[j].Valid = 0;
            cache.l2.ways[i].lines[j].Dirty = 0;
            cache.l2.ways[i].lines[j].Tag = 0;
            cache.l2.ways[i].lines[j].Time = 0;
            for (int k = 0; k < BLOCK_SIZE; k += WORD_SIZE)
            {
                cache.l2.ways[i].lines[j].Data[k] = 0;
            }
        }
        cache.l2.ways[i].init = 1;
    }
    cache.l2.init = 1;
...
void accessL2(uint32_t address, uint8_t *data, uint32_t mode)
{
    ...
    index = (address / BLOCK_SIZE) % (L2_SIZE / (BLOCK_SIZE * WAYS));
    tag = address / ((L2_SIZE / (BLOCK_SIZE * WAYS)) * BLOCK_SIZE);
    offset = address % BLOCK_SIZE;

    /* access Cache*/
    for (int i = 0; i < WAYS; i++)
    {
        CacheLine *Line = &cache.l2.ways[index].lines[i];
    ...
    // LRU
    uint32_t max = cache.l2.ways[index].lines[0].Time;
    int lru_index = 0;

    for (int i = 0; i < WAYS; i++)
    {
        uint32_t temp = cache.l2.ways[index].lines[i].Time;
        if (max < temp)
        {
            max = temp;
            lru_index = i;
    ...
    CacheLine *Line = &cache.l2.ways[index].lines[lru_index];
