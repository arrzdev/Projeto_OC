...
void initCache()
{
    ...
    {
        MultipleLineCache.lines[i].Valid = 0;
        MultipleLineCache.lines[i].Dirty = 0;
        MultipleLineCache.lines[i].Tag = 0;
        for (int j = 0; j < BLOCK_SIZE; j += WORD_SIZE)
        {
            MultipleLineCache.lines[i].Data[j] = 0;
        }
    }
    MultipleLineCache.init = 1;
}
...
void accessL1(uint32_t address, uint8_t *data, uint32_t mode)
{
    ...
    CacheLine *Line = &MultipleLineCache.lines[index];
